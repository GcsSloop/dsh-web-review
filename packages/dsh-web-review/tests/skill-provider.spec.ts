...
describe('bundled UI optimization Skill provider', () => {
  it('publishes all eight as user-invocable and only the Cordis selection as model-visible', async () => {
    ...
    registerUiSkillProvider(ctx, { autoLoadSkills: [...DEFAULT_AUTO_LOAD_SKILLS], previewCookies: true, browserPreview: true, browserExecutable: '', browserProfileDir: '', browserHeadless: true, browserViewportWidth: 1280, browserViewportHeight: 800 })
  ...
  it('honors a dynamically supplied selection and rejects duplicates', async () => {
    const ctx = new Context()
    await ctx.plugin(SkillRegistry)
    registerUiSkillProvider(ctx, { autoLoadSkills: ['better-colors'], previewCookies: true, browserPreview: true, browserExecutable: '', browserProfileDir: '', browserHeadless: true, browserViewportWidth: 1280, browserViewportHeight: 800 })
    ...
    expect(() => registerUiSkillProvider(ctx, { autoLoadSkills: ['better-ui', 'better-ui'], previewCookies: true, browserPreview: true, browserExecutable: '', browserProfileDir: '', browserHeadless: true, browserViewportWidth: 1280, browserViewportHeight: 800 }))
